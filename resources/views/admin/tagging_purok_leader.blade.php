...
<div class="mt-5 col-md-10">
    ...
    <form method="POST" action="{{ route('select.coordinator') }}">
        @csrf
                ...
                <tbody>
                    @foreach(session('purokLeaders') as $purokLeader)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('deletePurokLeader', $purokLeader->voter->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Purok Leader?');">
                                    @csrf
                                    @method('DELETE')
