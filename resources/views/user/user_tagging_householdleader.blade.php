...
<div class="mt-5 col-md-10">
    ...
    <form method="POST" action="{{ route('user_select_purok_leader') }}">
        @csrf
...
<div class="mt-3 col-md-12">
                    <form method="GET" action="{{ route('user_household_leader') }}">
    ...
    @else
        @foreach($voters as $voter)
            <tr>
                ...
                <td>
                    @if(session('selected_purok_leader'))
                        <form action="{{ route('user_storeHouseholdLeader') }}" method="POST">
                            @csrf
                        ...
                        @foreach($taggedMembers as $member)
                            @if($member->voter)
                                <tr id="row-{{ $member->voter->id }}">
                                    ...
                                    <td>
                                        <form action="{{ route('user_voter.destroy', $member->voter->id) }}" method="POST" 
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <!-- Members Table -->
                <form id="tagMembersForm" action="{{ route('user_tagHouseholdMembers') }}" method="POST">
                    @csrf
...
<script>
 $(document).ready(function () {
    $('#searchMembersForm').submit(function (e) {
        ...
        $.ajax({
            url: "{{ route('user_searchHouseholdMembers') }}",
            method: "GET",
            data: formData,
            beforeSend: function () {
...
$(document).ready(function () {
    ...
    $('#tagMembersForm').submit(function (e) {
        ...
        $.ajax({
            url: "{{ route('user_tagHouseholdMembers') }}",
            method: "POST",
            data: formData,
            headers: {
